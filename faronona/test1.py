...
#  python main.py -ai0 ./faronona/test1.py -ai1 ./faronona/random_agent.py -s 1 -t 1
...
#     return action_to_do
        ...
        def maxim (states, player, alpha, beta, depth):
            ...
            if terminal_test(states , player):
                print ("return since max_value terminated ")
                return utility(states , player)
            ...
            for move in possible_move:
                ...
                if FarononaRules.is_legal_move(states, move , player):
                    results = all_move(states,move, player, adver_die = 0)
                    ...
                    for result in results:
                        ...
                        if v > current_v:
                            current_v = v
                            if depth == 0:
                                self.move_choose[current_v] = move
        ...
        def minim (statesr, player , alpha, beta, depth):
            ...
            if depth >= 8:   # cutoff setting, maximum level AI can search through
            # if depth_of_game_tree >= 500:
                return evaluate(statesr , player)
            if terminal_test(statesr , player):
                print ("return since terminated")
                return utility(statesr , player)

            possible_move = FarononaRules.get_player_actions(statesr,player)
            current_v = float('inf')
            for move in possible_move:
                print ("in min_value body, action is choose from ", move, '\n\n')
                if FarononaRules.is_legal_move(statesr, move , player):
                    results = all_move(statesr,move, player, adver_die = 0)
                    # print("########### result depht player move ###########" , results,depht,player, move )
                    for result in results:
                        next_state = result["state"]
        ...
        def evaluate(state,player):
            ...
            print("########### EVTA ###########", adver_token_restant, me_token_restant,self.total_node_generated)
            ...
            req = (me_token_restant-adver_token_restant) * 1.0 / (me_token_restant+adver_token_restant)
            print("########### EVA ###########",req)
            print("########### EVA ###########", adver_token_restant, adver_token_restant)
            return req
