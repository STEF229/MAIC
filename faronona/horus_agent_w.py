...
class AI(FarononaPlayer):
    ...
    def play(self, state, remain_time):
        ...
        def best_move (stated , player, moves): # à optimiser ou créer une autre fonction de checking 
            ...
            if f is not None:
                corner_state = copy.deepcopy (stated)
                no_corner = 1
                next_stat, _a = FarononaRules.make_move( corner_state,f,player)

                info_me = next_stat.get_player_info(player)
                avant_corner = int(info_me["on_board"])
                l_mve = FarononaRules.get_player_actions(next_stat, opp_val)
                
                for l in l_mve:
                    net_state, _a = FarononaRules.make_move(next_stat,l, opp_val)
                    info_mee = net_state.get_player_info(player)
                    if int(info_mee["on_board"]) == avant_corner - 2 :
                        no_corner = 0
                if no_corner == 0 :
                    pass
                else:
